...
- [x] **Strategic-brainstorm capability shipped (2026-06-17d).** Added [[Strategic Brainstorm Protocol]] (charter — the read-only, *think-with-me* grounding session that complements `/vault-audit`: audit = vault **health**, brainstorm = strategic **completeness**) + the **`/os-brainstorm` skill** (`.claude/skills/os-brainstorm/`; tool-restricted read-only, **overrides §8 end-writes**). Built from an optimized "OS grounding & next-steps" prompt (5-layer reality scaffold; leans on the audit's F7 ship-coupling gap; authorship line / surfacing-not-deciding baked in). Wired into [[Collaboration Home]] ("Working modes") + [[Session Protocol]] (modes callout). **First `/os-brainstorm` run pending** — Orfeas to invoke next session.
