...
3. **One fact, one canonical owner.** Reference from many notes; maintain in one. (Commercial terms → the supplier dossier's private section · lifecycle → the canonical order-workflow note · state → [[Vault State Memory]] · priority → [[Capture Backlog]] · decisions → ADRs + session logs (the [[Decision Log]] was retired 2026-07-19) · metrics → generated analysis.)
