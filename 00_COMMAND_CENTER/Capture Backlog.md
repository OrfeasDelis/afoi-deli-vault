...
- [ ] **Open — decide at the next build session** (left open deliberately): **(1) the unit** — the **order** (status 0→4 fulfilment, the everyday Orfeas named) or the **quote** (sample-wall moment; Kronos cost chain already loaded, most "ready")? **(2) which real instance** — a concrete, recent, ordinary-with-a-wrinkle order/quote to use as the tracer.
