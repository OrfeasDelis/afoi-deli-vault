...
- [ ] **P2 polish (audit, remaining):** ✅ [[Weekly Review]] dormant-bannered *(2026-07-19b, revival condition: the data layer holds real rows)* · daily-note *system* dormancy note still open (template enum fixed) · tag question ([[Obsidian Tips and Tricks]] dead tag-search) · `created`/`updated` on every dossier for [[Supplier Enrichment Queue]] · ERD `PEOPLE`/`PAYMENTS` gap · remaining orphans beyond the named set · full [[README_START_HERE]] rewrite **(now scheduled: Pass 3, baseline §16-E)** · [[Vault Integrity Audit]] TODO — ✅ `/vault-audit` skill + `audit-critic` shipped *(2026-06-17)*; **write-time lint rules/hooks still pending** (the successor design is audit v2's session-close lint, baseline §11).
- [x] **P2 polish (2026-06-17 audit — F2/F3/F4) — CLEARED *(2026-07-19b, §16-C)*:** templates fixed (`Template - ADR` → `status: seed` · `Template - Project` → `status: idea` · same-class `Daily Note Template` → `seed`) · `AI Agent Roles` wired *(2026-06-19)* · `Supplier Note System` **superseded-marked** per OV-01 (deletion eligible after one audit cycle). Full list: [[2026-06-17-vault-audit]].
...
- [x] **Baseline (Pass 0–2, read-only, deep multi-agent)** — deterministic registries + 8 parallel domain readers + audit-critic verification (**67 CONFIRMED · 1 DEFLATED · 1 REJECTED**) → `docs/VAULT_BASELINE_2026-07-19.md` + `_meta/consolidation/` registries *(2026-07-19)*.
- [x] **Orfeas's §17 review — ALL 15 DECISIONS RULED *(2026-07-19b)*:** stratum **split** (facts stay live · voice lines to Orfeas) · Roadmap **rewrite instance-first** (after ADR-0008) · Current Priorities **careful diff, authored lines flagged** · Decision Log **retired** · Session Protocol **slim pointer** · Cost & Quote Build **reference-only** · one-fabric home = **CLAUDE.md** · overlap dispositions **approved** (OV-17's Cielo passage reserved for his call) · §16 sequence **approved** · generator redactions **approved** · roster column **kept** (pilot-scope) · Materials **pattern approved, batch/split deferred** · people notes **seed at Pass 4** · Weekly Review **dormant until the data layer lives** · sweep stash **kept until manifest-redo**. Rulings recorded per-cluster in [[Overlap Registry]] + [[Canonical Note Registry]].
- [x] **Mechanical batch (§16-C) — EXECUTED *(2026-07-19b)*:** phantom Builder's-Manual claims struck (VSM :46/:76 · Roadmap :13) · template enum fixes (F1/F2 + Daily) · dormancy/retirement/supersession banners (Weekly Review · Decision Log · Supplier Note System) · redoal raw-source banner · hub repairs (OV-13) · Selection Engine enum line · Vault Map `scripts/`+`_meta/consolidation/` rows + settled-P0-2 text · STATE.md ledger refresh + 2026-06-19 charter row · confidentiality redactions (CQB · Enrichment Queue :53 · VSM rows — OV-08/OV-07) · OV-15 cross-links · Strategic Axes 5-layer fix · one-fabric misattribution fix · scanner Greek/precision patterns · git-sync guard extended to the full docs/ suite · `/repo-analysis` contract fixes + regeneration.
- [ ] **Next (in order):** the **ADR batch** (§16-D — ADR-0008 instance-first first, then 0009 fields · 0010 verification tiers · 0011 external-system boundaries · the ADR-0005 amendment; governance rules A–J into `CLAUDE.md` after) → **Pass 3 steering** (§16-E — Roadmap fork · Current Priorities merge · QTR annotation with Orfeas · Session Protocol slim · README_START_HERE rewrite) → **Pass 4 = the tracer landing** (Priority 0.1 below, unchanged #1 content action).
...
> [!success] Interview COMPLETE — all seven batches A–G settled (2026-07-18)
> Ran as a **manual guided walk** (`/goal` confirmed **not** a registered command). Orfeas's pre-built **artifact reconstruction** (`Order Lifecycle — Ground-Truth Capture (Tracer v0.1)`, in `Downloads`) did the *artifact-reading half*; the **interview half** is captured in **[[Order Lifecycle — Ground-Truth Capture]]**: **A + B + C** (2026-06-19) · **D** loading/arrival (2026-06-21) · **E** receipt/delivery · **F** payment/invoice/margin · **G** special cases (all 2026-07-18) — **49 running schema-corrections**, spine captured end-to-end. **Next is NOT more interview — it's the final landing:** merge with the Tracer v0.1 draft → the full **S1–S15** narrative; turn the 49 corrections into concrete CSV/`.md` edits (new entities incl. **`stock_items`/inventory**, **`transport_claim`**, the **documents** model, **Kostas-Excel AP**, the **realized-margin** join — **F7 confirmed: per-order margin isn't computed today**); repoint the draft's dangling `[[order-fulfillment-workflow-normalized]]` → [[Order Workflow 0-4]]; exhibits into `_exhibits/tracer/`. **Parked deep-dives** (own sessions): Kostas's AP period-bending · a per-category lead-time table · the break-even-vs-payables pricing signal.
...
- [x] **Sequencing note — RESOLVED *(2026-07-19b)*:** instance-first **ruled** (baseline §17-2); **ADR-0008 files next** as §16-D's first act, then the Roadmap rewrite rides Pass 3. The **Builder's-Manual phantom struck** from [[Vault State Memory]] §4 + [[Roadmap]] L13 (§16-C — both now state it is planned, not existing).
