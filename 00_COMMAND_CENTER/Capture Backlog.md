...
- [ ] **⏭️ NEXT ACTION — run the first `/vault-audit`, before anything else.** The read-only audit system is now installed (the `/vault-audit` skill + the `audit-critic` adversarial verifier + scoped settings + the [[STATE|audit ledger]] — 2026-06-17). Run **`/vault-audit`** in a fresh session: it reconciles structure↔disk, refreshes the snapshot, and populates the ledger's Open list. **First target:** settle the **Contested** n8n question — [[Vault State Memory]] says the sweep is complete, but `CLAUDE.md §7` + the [[2026-06-16-vault-audit]] still flag ~15 notes. This **gates** Priority 0.5 (materials) and everything below until it's run.
