...
*Generated 2026-07-19b · Analysis v1.5 · snapshot ff3c968 · every number below is computed, never estimated · regenerated on every push/pull (`CLAUDE.md` §8, ADR-0006)*
...
| Measure | Value |
|---|---|
| Notes | 194 across 22 note-bearing folders (24 top-level directories) |
| Wikilinks | 1,962 total · 96.2% resolved · 1,178 unique note→note edges |
| Confidence-graded notes | 58 (`verified` 32 · `memory_seed` 12 · `likely` 10 · `needs_check` 4) |
| Top conceptual hubs | `Vault State Memory` · `Capture Backlog` · `The Heart` · `Supplier - Kronos` |
| Governing frame | **Consolidation & Enrichment Programme (ADR-0007)** — baseline reviewed 2026-07-19b: **all 15 decisions ruled, mechanical batch executed**; next: the ADR batch → Pass 3 → Pass 4 |
| Lead thread | The tracer — interview complete (A–G); **final landing = the programme's Pass 4** (49 corrections, mapped in the baseline) |
...
```mermaid
graph TD
    leads["Active leads"] ==> gate["GATE CLEARED ✅ — 15/15 review decisions ruled · mechanical batch executed"]
    gate ==> adrs["NEXT — the ADR batch · ADR-0008 instance-first first"]
    adrs ==> steer["THEN Pass 3 — steering rewrites · Roadmap fork · Priorities merge"]
    steer ==> tracer["THEN Pass 4 — the tracer final landing"]
    leads --> prog["Consolidation programme — ADR-0007 · baseline ✅ · review ✅ · batch C ✅"]
    leads --> mat["Materials layer — pattern approved ✅ · batching deferred behind the tracer"]
...
*✅ = complete. The programme's hard gate — baseline → review → execute — cleared on 2026-07-19b: Orfeas ruled all 15 review decisions, every ruling is recorded in the consolidation registries, and the approved mechanical batch landed the same session. The 49 schema-corrections stay mapped correction-by-correction in [docs/VAULT_BASELINE_2026-07-19.md](docs/VAULT_BASELINE_2026-07-19.md) §7; the CSVs change only at Pass 4, after ADR-0011.*
