...
*Generated 2026-07-18 · Analysis v1.3 · every number below is computed, never estimated · regenerated on every push/pull (`CLAUDE.md` §8, ADR-0006)*
...
| Measure | Value |
|---|---|
| Notes | 188 across 21 note-bearing folders (23 top-level directories) |
| Wikilinks | 1,757 total · 95.8% resolved · 1,039 unique note→note edges |
...
| Lead thread | The tracer — one real order end-to-end; **interview complete (A–G)**, final landing next |
...
graph TD
    ...
    subgraph mem["Steering and memory"]
        vsm["Vault State Memory · 19+ session logs · 6 ADRs"]
...
graph TD
    ...
    td --> te["E · receipt + delivery ✅"]
    te --> tf["F · payment + invoice + margin ✅"]
    tf --> tg["G · returns + edge cases ✅"]
    tg ==> landing["NEXT — the final landing · 49 corrections → schema-diff"]
