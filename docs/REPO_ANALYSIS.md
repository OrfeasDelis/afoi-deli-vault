...
**Analysis v1.3** · **Last analyzed:** 2026-07-18 · **Snapshot commit:** 1f0dd4c (+ the 2026-07-18 working tree — the tracer completion — committed immediately after; v1 baseline e686ba9)
...
This repository is the **Afoi Deli Second Brain** — a unified Obsidian vault serving as knowledge layer and operating system for both **Afoi Deli Floor + Bath** (ΑΦΟΙ ΔΕΛΗ Ε.Ε., a premium Athens floor/bath materials merchant, founded 1986) and **Orfeas Delis personally**, the second-generation successor; business and personal are deliberately one fabric (`The Heart.md`). It follows the LLM-Wiki pattern: immutable raw sources feed an LLM-maintained, densely interlinked wiki governed by a root contract (`CLAUDE.md`), with one hard override — doctrine, journal, and strategic framing are authored only by Orfeas. Structurally it is 188 notes across a numbered-folder taxonomy (business domains `00`–`11`, personal wing `12`–`17`, system layers `97`–`99`, `_meta`, `docs`), navigated by wikilinks rather than tags, with a session-ritual memory spine (`14_AI_COLLABORATION/Vault State Memory.md`) that makes every working session compound. Its strategic center of gravity is supplier intelligence (`04_SUPPLIERS_AND_BRANDS`) and the specification of a 5-layer "Afoi Deli OS" (Obsidian → Supabase Postgres → Python worker → interface → AI agent) of which — by the vault's own honest accounting — only Layer 1 currently runs.
...
| Wikilinks | **1,757** total · 1,683 resolved (95.8%) · 3 attachment links · 1 embed · 17 internal markdown links |
| Unique note→note edges | **1,039** |
...
| `status` distribution | active 92 · draft 44 · complete 25 · seed 10 · living 5 · idea 3 · backlog 2 · **off-set: `draft-for-verification` 1, `proposed` 1** |
...
| Note | In | Out | Degree |
|---|---|---|---|
| `14_AI_COLLABORATION/Vault State Memory.md` | 38 | 72 | 110 |
| `00_COMMAND_CENTER/Capture Backlog.md` | 33 | 67 | 100 |
| `_meta/audits/2026-06-16-vault-audit.md` | 14 | 40 | 54 |
| `The Heart.md` | 49 | 3 | 52 |
| `04_SUPPLIERS_AND_BRANDS/Suppliers/Kronos/Supplier - Kronos.md` | 31 | 14 | 45 |
...
The hub profile matches the declared architecture unusually well: the two state notes (memory + backlog) are the busiest crossroads, and `The Heart.md` has the signature of a true root — 49 inbound, only 3 outbound. A note pointed *to* from everywhere that points almost nowhere itself.
...
graph LR
    ...
    subgraph mem["Memory spine"]
        ...
        vsm --> sess["Session logs ×19 + brainstorms"]
        vsm --> adr["ADRs ×6"]
